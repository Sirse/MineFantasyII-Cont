...
public class MineFantasyHUD extends Gui {
    ...
    private static final ResourceLocation COGWORK_HELM_TEXTURE = TextureHelperMF
            .getResource("textures/gui/scopes/cogwork_helm.png");
    private static final ResourceLocation SCOPE_TEXTURE = TextureHelperMF
            .getResource("textures/gui/scopes/scope_basic.png");
    ...
    public void renderGameOverlay(float partialTicks, int mouseX, int mouseY) {
        ...
        if (mop != null) {
            if (mop.typeOfHit == MovingObjectPosition.MovingObjectType.ENTITY
                    && mop.entityHit instanceof EntityCogwork) {
    ...
    private void renderCogworkFuel(int width, int height, EntityCogwork suit) {
        ...
        this.drawTexturedModalRect(
                xPos + 6 + (CRAFT_METER_FG_WIDTH - progress),
                yPos + 11,
                CRAFT_METER_FG_U,
                CRAFT_METER_FG_V,
                progress,
    ...
    private void renderArmourRating(EntityPlayer player, int width, int height) {
        ...
        int baseAR = (player.ridingEntity instanceof EntityCogwork)
                ? ((EntityCogwork) player.ridingEntity).getArmourRating()
                : 0;
        float weight = (player.ridingEntity instanceof EntityCogwork)
                ? ((EntityCogwork) player.ridingEntity).getWeight()
                : 0;

        if (ArmourCalculator.advancedDamageTypes) {
            mc.fontRenderer.drawStringWithShadow(
                    StatCollector.translateToLocal("attribute.armour.protection"),
                    xPos,
                    yPos,
                    Color.WHITE.getRGB());
            ...
            mc.fontRenderer.drawStringWithShadow(
                    StatCollector.translateToLocal("attribute.armour.protection") + ": "
                            + ItemWeaponMF.decimal_format.format(generalAR),
                    xPos,
                    yPos,
                    Color.WHITE.getRGB());
        }
        ...
        mc.fontRenderer.drawStringWithShadow(
                CustomMaterial.getWeightString(weight),
                xPos,
                yPos + yOffset,
                Color.WHITE.getRGB());
    }
    ...
    private void displayTraitValue(int x, int y, int id, EntityPlayer player, int base) {
        ...
        String text = StatCollector.translateToLocal("attribute.armour.rating." + id) + " "
                + ItemWeaponMF.decimal_format.format(rating);
    ...
    private void renderStaminaBar(EntityPlayer player, int width, int height) {
        ...
        if (mc.currentScreen instanceof GuiInventory) {
            ...
            mc.fontRenderer.drawStringWithShadow(
                    stamTxt,
                    xPos + 41 - (mc.fontRenderer.getStringWidth(stamTxt) / 2),
                    yPos - 2,
                    bonus ? Color.CYAN.getRGB() : Color.WHITE.getRGB());
        }
    ...
    private void renderAnvilMetre(EntityPlayer player, TileEntityAnvilMF tile, int width, int height) {
        ...
        String s = knowsCraft
                ? (resultName.startsWith("gui.") ? StatCollector.translateToLocal(resultName) : resultName)
                : "????";
    ...
    private void renderGenericMetre(IBasicMetre tile, int width, int height) {
        ...
        this.drawTexturedModalRect(
                ...
                CRAFT_METER_FG_U,
                CRAFT_METER_FG_V,
                tile.getMetreScale(CRAFT_METER_FG_WIDTH),
                CRAFT_METER_FG_HEIGHT);
    ...
    private void renderQualityBalance(IQualityBalance tile, int width, int height) {
        ...
        this.drawTexturedModalRect(
                ...
                QUALITY_MARKER_U,
                QUALITY_MARKER_V,
                QUALITY_MARKER_WIDTH,
                QUALITY_MARKER_HEIGHT);
