 ...
 */
public class TileEntityCrossbowBenchRenderer extends TileEntitySpecialRenderer {

    private ModelCrossbowBench model;
    private Random random = new Random();

    public TileEntityCrossbowBenchRenderer() {
        model = new ModelCrossbowBench();
    }
    ...
    public void renderAModelAt(TileEntity tile, double d, double d1, double d2, float f) {
        ...
        for (int a = 0; a < 2; a++) {
            if (shouldRender(tile, a)) {
                this.renderModelAt("carpenter_crossbow", i, d, d1, d2, f, a);
            }
        }
    }
