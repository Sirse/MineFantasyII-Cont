...
public class RecipeHandlerCarpenter extends MFNEIRecipeHandler {
    ...
    private CachedCarpenterRecipe handleRecipe(ICarpenterRecipe irecipe) {
        if (irecipe instanceof ShapedCarpenterRecipes) {
            ...
            return new CachedCarpenterRecipe((TweakedShapedCBRecipes) irecipe);
        } else if (irecipe instanceof TweakedShapelessCBRecipes) {
            return new CachedCarpenterRecipe((TweakedShapelessCBRecipes) irecipe);
        }
        return null;
    }
    ...
    class CachedCarpenterRecipe extends CachedRecipe {
        ...
        public CachedCarpenterRecipe(TweakedShapedCBRecipes recipe) {
            carpRecipe = recipe;
            toolType = recipe.getToolType();
            toolTier = recipe.getRecipeHammer();
            benchTier = recipe.getAnvil();
        ...
        public CachedCarpenterRecipe(TweakedShapelessCBRecipes recipe) {
            carpRecipe = recipe;
            toolType = recipe.getToolType();
            toolTier = recipe.getRecipeHammer();
            benchTier = recipe.getAnvil();
        ...
        private void addIngredient(List<ItemStack> stacks, int x, int y) {
            ...
            for (ItemStack item : stacks) {
                if (minefantasy.mf2.api.heating.Heatable.canHeatItem(item)) {
