...
public class ConfigExperiment extends ConfigurationBaseMF {
    ...
    protected void loadConfig() {
        // RPGElements.isSystemActive = config.get(CATEGORY_EARLY,
        // "Enable Skills", false, "This will be a later feature where skills determine
        // a number of actions ingame, also will be used to unlock perks").getBoolean();
        stickArrows = config
                .get(
                        CATEGORY_ARCHERY,
                        "Save Arrows",
        ...
        dynamicArrows = config.get(
                ...
                .getBoolean();
        debugParry = config.get(
                CATEGORY_EARLY,
                "Debug Parry Logs",
                false,
                "Enable verbose parry debug output to log combat threshold calculations").getBoolean();
        ...
        ArmourCalculator.advancedDamageTypes = config.get(CATEGORY_ARMOUR, "Advanced Armour Calculator", true, AADesc)
                .getBoolean();
